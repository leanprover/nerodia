...
partial def highlightToml : Syntax → StateM (Option String) Highlighted := fun stx =>
  ...
  | .node info ``Lake.Toml.stdTable #[open1, contents, close1] => do
    let n := getKey ⟨contents⟩
    set n
    return srcInfoHl info <| .tableHeader <|
      .tableDelim ((← highlightToml open1)) ++
      (.tableName n (← highlightToml contents)) ++
      .tableDelim ((← highlightToml close1))
  ...
  | other => panic! s!"Failed to highlight TOML (probably highlightToml in needs another pattern case): {toString other}"
...
partial def Highlighted.toHtml (urlLinks : Bool := true) : Highlighted -> Html
  | .token t s =>
    match t with
  ...
  | .tableHeader hl =>
    {{<span class="table-header">{{hl.toHtml urlLinks}}</span>}}
  | .tableName _ hl =>
    hl.toHtml urlLinks
  | .tableDelim hl => {{<span class="table-delimiter">{{hl.toHtml  urlLinks}}</span>}}
  | .concat hls => .seq (hls.map (toHtml urlLinks))
  | .link url hl =>
    if urlLinks then
      {{<a href={{url}}>{{hl.toHtml urlLinks}}</a>}}
    else
      hl.toHtml urlLinks
  | .text s => s
  | .ws s =>
